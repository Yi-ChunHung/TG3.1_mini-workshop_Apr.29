...
function SuperCell()
    ...
    nvec    = [nv1 nv2 nv3];
    [~,I]   = sort(nvec,'descend');
    nbound  = ceil(nvec(I(1))) + ceil(nvec(I(2)));
    ...
    elseif isfield(instruct,'SPftn58sparse')
        ...
        ftn58sparse.BR = ftn58sparse.BR2D;
        isSlab      = 1;
    end
    ...
    %            for ss=1:size(ps,1)
    %                % build a larger table where atoms locate at nbound x nbound x nbound grid
    %                % the length unit is in bulk unitcell
    ...
    [ca, cb, cc] = ndgrid(-nbound:nbound, -nbound:nbound, -nbound:nbound);
    d            = [ca(:), cb(:), cc(:)];
    clearvars ca cb cc
    ps1          = cell(size(ps,1),1);
    for ss = 1:size(ps,1)
        ps_ss    = repmat([ps(ss, 1), ps(ss, 2), ps(ss, 3), ps(ss, 4), ps(ss,8)],length(d),1);
        ps1{ss}  = [ps_ss(:,1)+d(:,1), ps_ss(:,2)+d(:,2), ps_ss(:,3)+d(:,3), ps_ss(:,4:5)];
    end
    ps1          = cell2mat(ps1);
    clearvars d ps_ss
    ...
    for ii=1:size(ps1, 1)
        % if the atom in ps1 is in the "unit cell of the super cell", collect it into atomps
    ...
    % if there are atoms in the "super cell of the super cell", collect it into spatomps 
    ...
    if isSlab
        save slab_info.mat orbitps sporbitps sptable atomps BR NBR U2STR isSlab num_layer nv1 nv2 nv3 ribbon_direction
    else
        save slab_info.mat orbitps sporbitps sptable atomps BR NBR U2STR isSlab nv1 nv2 nv3 ribbon_direction
    end