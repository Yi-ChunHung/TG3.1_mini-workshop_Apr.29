...
for ik=1:nks
    ...
    H_SOC = RhSi_WSOC.RhSi_H(kpoints(ik,:));
    [vec, Etemp] = eig(H_SOC);
    Ek(ik,:)  = diag(Etemp);
...
hk=nks/(hs);
    for ii=1:norb
        plot(kd,Ek(1:nks,ii)-Ef,'Color',[0.600000023841858 0.200000002980232 0],'LineWidth',1.5);
        hold on
    end
...
F005  = fullfile(['Band_SOC'  '.fig']); 
F006  = fullfile(['Band_SOC'  '.png']);
saveas(h,F005)
saveas(h,F006)
